...
function DeveloperSettingsSection() {
  ...
  const checkHealth = async (showRefreshState = false) => {
    if (showRefreshState) {
    ...
    try {
      ...
      setFastApiStatus(res.ok ? "online" : "offline");
    } catch {
    ...
    try {
      ...
      setLlamaStatus(res.ok ? "online" : "offline");
    } catch {
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-primary/20 bg-primary/5 p-5 shadow-sm ring-1 ring-primary/20">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-1.5 opacity-80 mb-1">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            className="h-9 gap-2 shadow-sm border-primary/20 hover:bg-primary/10"
          ...
          <Button variant="default" className="h-9 gap-2 shadow-sm" onClick={() => navigate("/settings/api-logs")}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-primary/10 pt-4">
        <div className="flex items-center justify-between rounded-lg border border-primary/10 bg-background/50 px-3 py-2.5">
          ...
          <div className="flex items-center gap-1.5">
            <span className={cn("h-2 w-2 rounded-full", fastApiStatus === "checking" ? "bg-muted-foreground animate-pulse" : fastApiStatus === "online" ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-destructive")} />
            <span className={cn("text-xs uppercase tracking-wider font-bold", fastApiStatus === "online" ? "text-green-600 dark:text-green-500" : fastApiStatus === "offline" ? "text-destructive" : "text-muted-foreground")}>
              {fastApiStatus}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-lg border border-primary/10 bg-background/50 px-3 py-2.5">
          ...
          <div className="flex items-center gap-1.5">
            <span className={cn("h-2 w-2 rounded-full", llamaStatus === "checking" ? "bg-muted-foreground animate-pulse" : llamaStatus === "online" ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-destructive")} />
            <span className={cn("text-xs uppercase tracking-wider font-bold", llamaStatus === "online" ? "text-green-600 dark:text-green-500" : llamaStatus === "offline" ? "text-destructive" : "text-muted-foreground")}>
              {llamaStatus}
            </span>
          </div>
        </div>
